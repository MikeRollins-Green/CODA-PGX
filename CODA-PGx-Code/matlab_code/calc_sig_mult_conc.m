function signi=calc_sig_mult_conc(posr,ncr)
...
for nn=1:tt
    ...
    oo=find(strcmp(posr.drug,posr.drug(nn))&strcmp(posr.cell_line,posr.cell_line(nn)));
    ...
    if sort(unique(temp_pools))==[1;2]
        ...
        signi.collabels=[signi.collabels;{[posr.cell_line{nn} '-' posr.drug{nn}]}];
        
        uniq_tag=[uniq_tag; {[char(posr.drug(nn)) char(posr.cell_line(nn))]}];
