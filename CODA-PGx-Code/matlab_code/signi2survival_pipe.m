function fid_str=signi2survival_pipe(signi,clin,drugs,gof,lof,gg,min_fc,max_p_signi,minNpatients,max_treat_line,max_p_km,look,min_abs_J)
...
for nn=1:tt
    ...
    if nc>=minNpatients
        run_em(drugs,clin,lof,my_genes,tcga_drug,tc,max_treat_line,max_p_km,'LOF',min_abs_J,max_p_signi,fid);
        run_em(drugs,clin,gof,my_genes,tcga_drug,tc,max_treat_line,max_p_km,'GOF',min_abs_J,max_p_signi,fid);
        run_em(drugs,clin,gg,my_genes,tcga_drug,tc,max_treat_line,max_p_km,'MUT_ONLY',min_abs_J,max_p_signi,fid);
        
        run_em_anti_hit(drugs,clin,lof,my_genes_anti_hit,tcga_drug,tc,max_treat_line,max_p_km,'LOF',min_abs_J,max_p_signi,fid);
        run_em_anti_hit(drugs,clin,gof,my_genes_anti_hit,tcga_drug,tc,max_treat_line,max_p_km,'GOF',min_abs_J,max_p_signi,fid);
        run_em_anti_hit(drugs,clin,gg,my_genes_anti_hit,tcga_drug,tc,max_treat_line,max_p_km,'MUT_ONLY',min_abs_J,max_p_signi,fid);
    end
...
function run_em(drugs,clin,lof,my_genes,my_drug,tc,max_treat_line,max_p_km,my_ptype,min_abs_J,print_pp,fid)
...
for nn=1:length(my_genes)
    ...
    if p<=max_p_km && abs(J)>min_abs_J
        fprintf(fid,[my_genes{nn} '\t' 'HIT' '\t' num2str(print_pp) '\t' char(my_ptype) '\t' char(my_drug) '\t' char(tc) '\t' num2str(max_treat_line) '\t' num2str(p) '\t' num2str(J) '\n']);
    end
    
end

function run_em_anti_hit(drugs,clin,lof,my_genes,my_drug,tc,max_treat_line,max_p_km,my_ptype,min_abs_J,print_pp,fid)
...
for nn=1:length(my_genes)
    ...
    [J,p]=analyze_em_no_km(drugs,clin,lof,my_genes{nn},my_drug,tc,max_treat_line);
    if p<=max_p_km && abs(J)>min_abs_J
        fprintf(fid,[my_genes{nn} '\t' 'ANTIHIT' '\t' num2str(print_pp) '\t' char(my_ptype) '\t' char(my_drug) '\t' char(tc) '\t' num2str(max_treat_line) '\t' num2str(p) '\t' num2str(J) '\n']);
    end
